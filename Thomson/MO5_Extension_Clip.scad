...
difference() {
    union() {
       translate([0,0,2]) cube([53.5-0.5,32,2+2], center=true);
        for (dx=[-1,1])
            translate([dx*(46.5+10)/2,-0.5,4/2]) cube([10,8,4], center=true);
        for (dx=[-1,1],dy=[-1,1]) {
            translate([dx*(46.5+3.5-1.5)/2,dy*8-4,4]) rotate([0,0,0]) linear_extrude(5, scale=[0.8,0.9]) square([4.5,1.6], center=true);
        }
    }
