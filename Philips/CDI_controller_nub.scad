    ...
    difference() {
        ...
        difference() {
            union() {
                ...
                if (optimize_fdm)
                    translate([0,0,9.58-5.35-.99]) cylinder(d1=d1/6,d2=d1,h=1);
            }
            if (optimize_fdm) for (a=[0:120:359])
                rotate([0,0,a]) translate([d1/2,0,0]) cube([0.4,0.5,20], center=true);
        }
